...
for /F "tokens=*" %%A in (done1.txt) do (
	echo %%A

	dir /S /B /ON %%A\%sourcepath%\*.png > done1frames.lst
	REM where AR01_3\neat_smoother_2x_selected\*.png > frames.lst
	..\radtools\radvideo64 binkc done1frames.lst /F15 /P32 %%A.bik /#
	...
	if exist ..\VQA\%%A.wav (
		...
		REM ffmpeg -framerate 15 -i %%A\%sourcepath%\%%A.vqa_%%04d.png -i ..\VQA\%%A.wav -c:v libx264 -preset fast -crf 10 -maxrate 50M -bufsize 25M -pix_fmt yuv420p -movflags faststart -tune fastdecode  "%%A%outputpostfix%.mp4"
		...
		REM ffmpeg -framerate 15 -i %%A\%sourcepath%\%%A.vqa_%%04d.png -c:v libx264 -preset fast -crf 10 -maxrate 50M -bufsize 25M -pix_fmt yuv420p -movflags faststart -tune fastdecode "%%A%outputpostfix%.mp4"
	)
)
del done1frames.lst