...
For $filename In $filelist	
	If Not FileExists("D:\br_re\VQA\" & $filename & ".avi") Then
		...
		;If Not WinActive("Open","") Then WinActivate($title,$text)
		...
		;MouseClick("left",283,282,1)
		;MouseClick("left",356,322,1)
		;Send("{ALT}c")
		Send("&c")
		Sleep(100)
		Send("{DOWN}") ; AVI
		...
		;MsgBox($MB_SYSTEMMODAL, "", "converting1")

		;_WinWaitActivate("VQAtoAVI 2","", 100) ; pcx
		Sleep(100)

		If WinActive("VQAtoAVI 2","") Then Send("{ENTER}")
		...
		While Not ControlCommand('VQA to AVI Converter written by Tim Kosse', '', 'Button6', 'IsEnabled', '')
			Sleep(500)
		WEnd
		;MsgBox($MB_SYSTEMMODAL, "", "converting2")
	EndIf
Next
